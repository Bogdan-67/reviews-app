...
const CreatePollForm = forwardRef(
  (
    { title = true, showOkBtn = true, onSuccess, onError, id_request }: Props,
    ref
  ) => {
    const {
      handleSubmit,
    ...
    const submit: SubmitHandler<IPoll> = async (data) => {
      console.log(data);
      setIsSending(true);
      await PollService.createPoll(JSON.stringify(data), id_request)
        .then((response) => {
